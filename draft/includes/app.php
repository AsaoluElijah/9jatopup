<?php
    ...
    class App extends Connect{
        ...
        function setCookie($value,$days){
            // 86400 = 1 days
            $cookie = setcookie('9JATOPUP',$value, time() + (86400 * $days), "/");
            return $cookie;
        }

        function setCookieA($value,$days){
            // 86400 = 1 days
            $cookie = setcookie('NaijaTopup ',$value, time() + (86400 * $days), "/");
            return $cookie;
        }
        ...
        function deduct($amount,$userId){
            ...
            if ($row > 0) {
                ...
                $amountBefore = $row['amount'];
                $newAmount = $amountBefore - $amount;
                $sql = "UPDATE user SET amount = $newAmount WHERE id = '$userId' ";
        ...
        function buyAirtime($network,$price){
            ...
            if ($row <= 0) {
                ...
                // $userWallet = $_COOKIE['9JATOPUP'];
                // $this->deduct($price,$userWallet);
                
                $cardId = $row['id'];
                $sql2 = "UPDATE $network SET status = 'used' WHERE id = $cardId ";
